...
// ── Generic proxy — handles all /api/bms/* ───────────────────────────────────
...
// ── Send Raut bill via BMS (creates BMS invoice + sends email) ───────────────
const sendBillViaBMS = async (req, res) => {
  ...
  try {
    // 1. Fetch Raut bill with line_items
    const bill = await billsService.getBillById(billId)
    if (!bill) return res.status(404).json({ success: false, message: 'Bill not found' })

    // 2. Create BMS invoice from Raut bill data
    ...
    if (!bmsInvoiceId) {
      return res.status(500).json({ success: false, message: 'BMS invoice created but ID not returned' })
    }

    logger.info(`✅ BMS invoice created: ${bmsInvoiceId} for Raut bill #${bill.bill_no}`)

    // 3. Send via BMS email
